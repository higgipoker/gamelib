...
static std::map<int, std::string> buttons {
    {0, "A"},     {1, "B"},     {2, "X"},       {3, "Y"},       {4, "LB"},    {5, "RB"},     {6, "LTRIGER"}, {7, "RTRIGGER"}, {8, "BACK"},
    {9, "START"}, {10, "XBOX"}, {11, "LCKICK"}, {12, "RCLICK"}, {13, "LEFT"}, {14, "RIGHT"}, {15, "UP"},     {16, "DOWN"},
};
...
void XboxController::Update() {
    ...
    for (unsigned int i = 0; i < 100; ++i) {
        if (sf::Joystick::isButtonPressed (sf_joystick_index, i)) {
            const int key = static_cast<const int> (i);
            if (buttons.find (key) != buttons.end())
                std::cout << buttons[key] << std::endl;
            //        else

...
// Triangle
// ------------------------------------------------------------
void Primitives::Triangle(Window &target, unsigned int x1, unsigned int y1, unsigned int x2, unsigned int y2,
...
// Arc
// ------------------------------------------------------------
void Primitives::Arc(Window &target, float x, float y, float radius, float start, float end, int segments,
					 int thickness) {
	...
	for (unsigned int i = 0; i < thickness; ++i) {
		...
		while (current <= act_end) {
			...
			vertices.push_back(sf::Vertex(
			 sf::Vector2f(ax, ay), sf::Color(outline_color.r, outline_color.g, outline_color.b, outline_color.a)));

...
// constructor
// ------------------------------------------------------------
GameLib::Window::Window(const std::string &title, unsigned int x, unsigned int y, unsigned int w, unsigned int h, bool fullsscreen) {
	// default style
...
WindowEvent Window::PollEvent(void) {
	...
	while (window.pollEvent(sfml_event)) {
		...
		if (sfml_event.type == sf::Event::Closed) {
			...
			wnd_event.param = IntToString(sfml_event.mouseWheel.delta);
		}
	}

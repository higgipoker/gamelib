...
Tileset::Tileset(const std::string &filename, Camera &c) : GameLib::Sprite(filename, 1, 1), floor_width(10000), floor_height(10000), camera(c) {
    ...
    tiles_wide = floor_width / static_cast<unsigned int>(sprite.getTextureRect().width);
    tiles_high = floor_height / static_cast<unsigned int>(sprite.getTextureRect().height);

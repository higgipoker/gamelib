...
bool Point::inRect(const Rectangle &_rect) const {
  if (x > _rect.x && x < _rect.x + _rect.w && y > _rect.y &&
      y < _rect.y + _rect.h) {
    return true;
  } else {
    return false;
...
bool Rectangle::overlaps(const sf::FloatRect &_rect) const {
  if (this->x < _rect.left + _rect.width && this->x + this->w > _rect.left &&
      this->y < _rect.top + _rect.height && this->y + this->h > _rect.top) {
    return true;
  }
